...
import {RegisterSuccess, Regis, RegisterFail} from "../actions/auth.action";
...
export default function SignUp() {
    ...
    const onRegister = async (event) => {
        ...
        try {
            const register = await SignUpWithUsernameAndPassword(information);
            if (register?.userData) {
                dispatch(RegisterSuccess(register?.userData));
                setSuccess(true);
                setOpen(true);
            }
        } catch (error) {
                    ...
                    </Typography>
                    {
                        (success === true && open) && (
                            <Alert variant="outlined" severity="success" onClose={() => {
                                ...
                                Register successfully, back to <Link href="/login" variant="body2"> Log in</Link>
                            </Alert>
                        )
                    }
                    {
                        (auth.error != null && open) && (
